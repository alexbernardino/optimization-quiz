...
QUIZ_URL = "https://alexbernardino.github.io/optimization-quiz/"
OUTPUT = Path(__file__).resolve().parent.parent / "optimization-quiz-qr.png"
MODULE_PIXELS = 16
    ...
    for row in range(modules):
        for column in range(modules):
            if qr.isDark(row, column):
                ...
                draw.rectangle(
                    (x, y, x + MODULE_PIXELS - 1, y + MODULE_PIXELS - 1),
                    fill=0,
                )

    image.save(OUTPUT, optimize=True)
    print(f"Created {OUTPUT} ({side} x {side} px) for {QUIZ_URL}")
